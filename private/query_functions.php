<?php
  ...
  function insert_user($user) {
    ...
    $stmt = mysqli_prepare($db, "INSERT INTO users SET first_name=?, last_name=?, email=?, username=? created_at=?");
     $stmt->bind_param("sssss", $user['first_name'], $user['last_name'], $user['email'], $user['username'], $user['id'], $created_at);
     $stmt->execute();
  ...
  function update_user($user) {
      ...
      $stmt = mysqli_prepare($db, "UPDATE users SET first_name=?, last_name=?, email=?, username=? WHERE id=? LIMIT 1");
       $stmt->bind_param("sssss", $user['first_name'], $user['last_name'], $user['email'], $user['username'], $user['id']);
       $stmt->execute();
    //
    // $errors = validate_user($user);
    // if (!empty($errors)) {
  ...
  function delete_user($user) {
    global $db;
    $stmt = mysqli_prepare($db, "DELETE FROM users WHERE id = ? LIMIT 1");
    $stmt->bind_param("s", db_escape($db, $user['id']));
    $stmt->execute();
